...
knitr::include_graphics("../Results/v1.0/SE_Trout_segments_QGIS.map.jpeg")
```
...
## Hierarchical Model
We used a Bayesian hierarchical model to infer synchrony in brook trout populations and to identify climate drivers of synchronous dynamics [@berliner1996: @wikle1998]. The N-mixture data model [@royle2004] coupled with a log linear process model allowed us to quantify climate effects on abundance. We modeled the natural logarithm of brook trout density (fish/1000 m$^2$) as a function of summer temperature and winter streamflow. We analyzed counts of brook trout abundance from 169 unique sites over a 34-year period from 1982 to 2015.  
...
where $a_i$ is the sum of site areas (length $*$ median wetted width) of the given stream segment. Local density was represented as a function of segment-level climate covariates $\bm{x}_{i,t}$ using the log-link function
...
where $\alpha_i$ represents average log density at site $i$ when climate variables are set at their mean and $\bm{\beta}_i$ represents a vector of site-specific covariate effects. The terms $\epsilon_t$ and $\gamma_{i,t}$ represent spatially- and spatiotemporally-structured random effects, respectively. Priors on $\alpha_i$ and $\beta_i$ were noninformative: $\alpha_i \sim \text{normal}(0, 1000)$ and $\beta_i \sim \text{normal}(\mu_\beta, \sigma^2_\beta)$ where $\mu_\beta \sim \text{normal}(-0.5, 100)$ and $\sigma_\beta \sim \text{uniform}(0, 10)$. The random effects are assumed to be independent with distributions $\epsilon_t \sim \text{normal}(0, \sigma^2_\epsilon), \sigma^2_\epsilon \sim \text{uniform}(0, 10)$, $\gamma_{i,t} \sim \text{normal}(0, \sigma^2_{\gamma,i})$ and $\sigma_{\gamma,i} \sim \text{uniform}(0, 10)$. The $\epsilon_t$ term represents the between-year variation in density that is synchronous to all segments, and the $\gamma_{i,t}$ term represents the variation that is segment-specific (i.e., asynchronous).  
...
The synchronizing or asynchronizing effects of the climate covariates can be estimated from the variances of the covariate coefficients ($\beta$s). A coefficient with low variance (that is, similar effects on different populations) is one that has a synchronizing effect. A covariate with high variance (differing effects on different populations) is one with an asynchronizing effect. In calculating this variance, some authors have taken the variance of the product of covariate coefficient and the average value of the climate variable across sites at each year [@nakagawa2013; @ghislain2022 (in review)]. We demonstrate that since climate is highly synchronous in space, this additional step is unwarranted in our case.

We fit a total of six models: One model each for YOY and adults at the regional scale fit to all stream segments (hereafter the “regional” models), as well as one model each for YOY and adults in the northern and southern halves of the study region, hereafter the “subregional” models. We implemented our models utilizing Markov Chain Monte Carlo (MCMC) sampling using JAGS with the 'jagsUI' package in R [@kellner2021]. We provide code in Appendix \@ref(code-for-n-mixture-model). After a burn-in period of 25,000 samples (50,000 for subregional models), three chains were run until 100,000 iterations were reached. To evaluate the performance of our models, we completed posterior predictive checks for  mean and coefficient of variation. We report posterior means as point estimates and 95% highest posterior density credible intervals as estimates of uncertainty.