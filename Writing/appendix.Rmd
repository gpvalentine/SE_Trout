...
```{r temp-sites-map, fig.cap="Locations of 204 paired air and water temperature sensors."}
knitr::include_graphics("../Results/v2.0/NS204_temp_sites_map.jpeg")
```

\newpage

```{r precip-sites-map, fig.cap="Locations of 51 precipitation measurement sites. Source: NOAA NCEI"}
...
```{r YOY-betas-corrPlot-plot, fig.cap="Spearman correlations between segment-specific climate effects on YOY log density (model $\bf{\beta}_i$s) and segment geography."}
...
kable(YOY_beta_corrs.table,
      col.names = c("Climate Covariate", "Local Habitat\nVariable", "Spearman Correlation"),
      caption = "Top ten correlations between model $\\bm{\\beta}_i$s and local habitat.")
```

\newpage

```{r YOY-climate-effects-semivariogram-plot, fig.cap = "Semivariogram depicting spatial structure in stream segment-specific climate effects on abundance of young-of-year brook trout. If spatial strucutre is present, nearer values will have lover semivariance than farther values. Climate variables include a) average 0.9Q summer air temperature (year t-1), b) max 0.9Q winter stream flow (year t), and c) max 0.9Q spring stream flow (year t). DD = Decimal degrees."}
...
kable(YOY_ICC_corrs.table,
      col.names = c("Local Habitat\nVariable", "Spearman Correlation"),
      caption = "Top ten correlations between ICCs and local habitat.")
```

\newpage

```{r YOY-ICC-semivariogram-plot, fig.cap="Semivariogram depicting spatial structure in stream segment-specific intraclass corrrelation coefficients of abundance of young-of-year brook trout. If spatial strucutre is present, nearer values will have lover semivariance than farther values. DD = Decimal degrees."}
YOY_ICCs_semivariogram.plot
```

\newpage

```{r stock-recruitment-plot, fig.cap = "Semivariogram depicting spatial structure in stream segment-specific intraclass corrrelation coefficients of abundance of young-of-year brook trout. If spatial strucutre is present, nearer values will have lover semivariance than farther values. DD = Decimal degrees.", message=FALSE, warning=FALSE}
plot_grid(SSR_1.plot,
          SSR_2.plot,
          labels = c("a)", "b)"),
          nrow = 1)
```
